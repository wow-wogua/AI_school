...
围绕「**采集数据 → 形成成长档案 → AI 分析 → 教师干预 → 生成成长报告**」建设的初中素质报告平台。
...
- **AI 后台任务队列**：寄语/总结提交后切页面、关浏览器照跑不中断，回来自动恢复；支持整班批量生成，顶部「AI 任务」面板统一查看进行中/已完成任务；多教师共用队列，并发贴合模型供应商限额
