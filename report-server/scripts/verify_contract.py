...
3. 聚合 JSON 走渲染核心 → PDF，页数 53（52 内容页+空白尾页：批7 重立基线——学生1 新增 4 条微光
   记录触发「成长掠影」可选页 +1；尾页换校批次起存在，基线同口径）且每页中文大标题序列与 target/report.pdf 一致
...
def main():
    ...
    ok = not out and n == 53 and sig == tsig
